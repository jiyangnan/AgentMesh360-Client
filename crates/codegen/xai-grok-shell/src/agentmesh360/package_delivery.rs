...
mod tests {
    ...
    fn older_install_refresh_cannot_overwrite_a_newer_active_version() {
        ...
        let older = install_direct(
            &service,
            verified_download_from_manifest(temp.path(), &runtime_upgrade_manifest("0.5.7"), 'd'),
        );
        let newer = install_direct(
            &service,
            verified_download_from_manifest(temp.path(), &runtime_upgrade_manifest("0.5.8"), 'e'),
        );
    ...
    fn rollback_refreshes_catalog_and_preserves_the_stable_main_session() {
        let temp = tempfile::tempdir().expect("tempdir");
        let service = PackageDeliveryService::in_home(temp.path());
        deliver_manifest(&service, &runtime_upgrade_manifest("0.5.7"), 'f');
        deliver_manifest(&service, &runtime_upgrade_manifest("0.5.8"), 'g');
        ...
        assert_eq!(receipt.version, "0.5.7");
        assert!(matches!(
            receipt.runtime_visibility,
            PackageRuntimeVisibility::Visible { .. }
        ));
    ...
    fn rollback_rejects_invalid_access_and_damaged_previous_without_mutation() {
        let temp = tempfile::tempdir().expect("tempdir");
        let service = PackageDeliveryService::in_home(temp.path());
        deliver_manifest(&service, &runtime_upgrade_manifest("0.5.7"), 'h');
        deliver_manifest(&service, &runtime_upgrade_manifest("0.5.8"), 'i');
        let before = service
            .installer
            .get(PACKAGE_ID)
            .expect("registry")
    ...
    fn committed_rollback_reports_pending_until_catalog_reconciliation_recovers() {
        let temp = tempfile::tempdir().expect("tempdir");
        let service = PackageDeliveryService::in_home(temp.path());
        deliver_manifest(&service, &runtime_upgrade_manifest("0.5.7"), 'j');
        deliver_manifest(&service, &runtime_upgrade_manifest("0.5.8"), 'k');
        ...
        assert_eq!(pending.version, "0.5.7");
        assert!(matches!(
            pending.runtime_visibility,
            PackageRuntimeVisibility::RefreshPending { .. }
        ));
        ...
        assert_eq!(recovered.version, "0.5.7");
        assert!(matches!(
            recovered.runtime_visibility,
            PackageRuntimeVisibility::Visible { .. }
        ));
    ...
    fn rollback_waits_for_the_shared_package_mutation_gate() {
        let temp = tempfile::tempdir().expect("tempdir");
        let service = PackageDeliveryService::in_home(temp.path());
        deliver_manifest(&service, &runtime_upgrade_manifest("0.5.7"), 'm');
        deliver_manifest(&service, &runtime_upgrade_manifest("0.5.8"), 'n');
    ...
    fn seed_remote_package(
        ...
        let release_url =
            format!("{PRODUCTION_PACKAGE_ORIGIN}/{PACKAGE_ID}-0.5.6.agent-release.v1.json");
        let artifact_url = format!("{PRODUCTION_PACKAGE_ORIGIN}/{PACKAGE_ID}-0.5.6.ampkg.tar.zst");
        let envelope_url =
            format!("{PRODUCTION_PACKAGE_ORIGIN}/{PACKAGE_ID}-0.5.6.signature.v1.json");
        let trust =
