...
| 旅程 | 用户目标 | 核心用例 |
| --- | --- | --- |
| 信息架构和引导 | 一眼知道去哪里配置、管理和继续工作 | TC-NAV-001 ～ 003、TC-GUIDE-001 ～ 002 |
...
| 左侧导航与引导 | 唯一 Agent 一级入口、左下账户入口、Host 状态、三步引导 | 默认落点、头像键盘入口、唯一 active、Host 三态、加载/失败、零 Provider、零 Agent、激活/失效/回访、恢复引导 | Node + Electron + 安装包；TC-NAV-001～003、TC-GUIDE-001～002 |
...
| 账户与设置 | 模型供应商、账号与订阅、后台运行、使用指南、高级诊断 | 五子页、Provider 懒加载、键盘/焦点、账号切换、登录启动、Host 异常、诊断不可用 | Node + Electron + 安装包；TC-HOST-001～007、TC-SETTINGS-001 |
| 内部交付 | DMG/ZIP、receipt、SHA、单包留存 | clean pushed commit、失败保留旧包、生产发布门关闭 | Node + 安装包；TC-RELEASE-001～003 |

## 3.2 信息架构和首次引导

## TC-NAV-001：唯一 Agent 一级导航与账户设置入口

- **用户故事**：作为普通用户，我希望打开客户端后直接使用 Agent；只有需要配置时，才从
  账户入口进入模型供应商和客户端设置。
- **优先级**：P0
- **设计状态**：已实现
- **前置条件**：已登录且订阅有效。
- **输入**：启动客户端；用鼠标与 Space/Enter 操作左下账户区域；返回 Agent。
- **交互步骤**：检查默认页面和全部 `.nav-item`；点击账户区域；依次打开内部五项；点击 Agent
  返回；进入和退出某个 Agent。
- **预期输出**：一级导航恰好只有 Agent 且默认进入 Agent 列表；不存在公开的
  `nav-providers`、`nav-settings`、“首页”“当前对话”或“Agent Package”；账户区域是具有
  “打开账户与设置”中文名称的 button，点击后显示模型供应商、账号与订阅、后台运行、使用指南、
  高级诊断，恰好一个 active 和 `aria-current=page`；退出登录只在账号与订阅页；打开常驻
  Agent 后默认就是对话，模型、行为和用户偏好只从右上角齿轮进入。
- **失败与恢复**：未知旧路由回退到 Agent 列表，不出现空白页或重复入口。
- **验证层**：Node + Electron + 安装包 + 人工
- **本轮结果**：待执行
- **证据说明**：源码 Node/Electron 已通过；新信息架构的安装包动态验收和 owner 人工确认
  尚待本轮新包完成，不能沿用旧包结论。
...
- **预期输出**：显示清楚的中文三态；点击进入“账户与设置 > 后台运行”；不会用技术错误覆盖主内容。
...
- **优先级**：P0
- **设计状态**：已实现
- **前置条件**：有效订阅；Host Overview 可返回已配置 Provider 数、Agent 模型摘要和
  `bindingIssue`，但不返回 Provider Profile 或凭据。
- **输入**：依次构造零 Provider、已有 Provider 但未激活、激活 IPC 在途、运行态 `starting`、
  常驻 Agent 模型失效、正常常驻 Agent，以及正常/失效 Agent 混合状态。
- **交互步骤**：打开 Agent 默认页；按当前唯一 CTA 配置 Provider、聚焦可激活 Agent 或进入
  模型修复；激活完成后返回首页；从“账户与设置 > 使用指南”再次查看。
- **预期输出**：顺序始终为“添加模型供应商 → 激活 Agent → 在 Agent 对话中开始工作”；
  ready 状态恰好一个当前步骤和一个真实 CTA；零 Provider 不提前读取完整 Provider Snapshot；
  失效 Agent 只进入自身模型设置；正常常驻 Agent 的回访首页不显示新手卡，设置指南显示
  “打开或继续 + 明确 Agent 名称”，且第三步保持“当前步骤”而不伪造已经发过消息。
- **失败与恢复**：普通切页与订阅静默复验不重置 Provider、Agent 或会话；Agent 激活 IPC
  返回后，身份非 ready、存在激活错误、目标未进入 `desiredState=running` 或运行态不属于允许的
...
- **用户故事**：作为首次用户，我不希望客户端在状态未知时误导我，也不希望点击一个没有
  结果的入口。
- **优先级**：P0
- **设计状态**：已实现
- **前置条件**：有效订阅；Overview 可延迟、失败或返回空 Agent Catalog。
- **输入**：延迟 Overview、一次失败后成功、Provider 已配置但 Agent 列表为空、迟到旧请求、
  账号切换、1180×760 与 720×760 窗口。
- **交互步骤**：观察读取中状态；触发失败；点击重试；在空目录状态点击重新确认；确认 CTA
  键盘可达并可用 Space 激活；检查窄窗口。
- **预期输出**：读取中三步均为“待确认”、`aria-busy=true`、零 CTA；失败时零
  `aria-current` 且只有重试；空 Catalog 不显示“查看可激活 Agent”的静默按钮；异步区域
  `aria-live=polite`；CTA 至少 44px/13px，页面无横向溢出。
- **失败与恢复**：旧请求、旧账号或失败结果不得覆盖当前账号；普通重试只读 Overview，空目录
  恢复只重新读取 Host Agent Catalog，均不写 Provider、Agent 或 Session。
- **验证层**：Node + Electron + 安装包
- **本轮结果**：通过
- **证据说明**：源码状态矩阵、迟到响应、键盘激活、窄窗口和真实 Catalog 刷新通过；新包
  `app.asar`、真实 Host、DMG/ZIP 与 receipt 通过。现有安装按安全合同未被自动覆盖。
...
- **预期输出**：同账号普通导航/复验/暂时关闭保留 Renderer 临时草稿；明确放弃和换账号清空；
  ...
  名称不小于 16px，状态/元信息/操作不小于 13/12/13px，普通文案统一 UI 字体；Provider
  列表、删除影响查询、Probe 和增删改操作的旧账号迟到结果均不得写入新账号页面。
- **失败与恢复**：Host 删除失败时列表不变并显示可重试错误；完整 Key 不进入 HTML、正文、
  日志或公开配置；账号切换使在途 Provider UI 写回失效，但不伪造旧操作的成功或失败提示。
...
## TC-HOST-003：账户与设置 > 后台运行的系统登录启动开关

- **用户故事**：作为用户，我希望自己决定开机后 Agent 是否在后台恢复。
- **优先级**：P1
- **设计状态**：部分实现
- **前置条件**：“账户与设置 > 后台运行”页；内部 unsigned build。
- **输入**：开启/关闭“登录时启动”。
...
## TC-SETTINGS-001：账户设置中心五类子菜单

- **用户故事**：作为用户，我希望日常工作区只突出 Agent，需要配置时再从左下账户入口集中管理
  Provider、账号、后台、指南和诊断。
- **优先级**：P1
- **设计状态**：已实现
- **前置条件**：已登录且订阅有效。
- **输入**：点击左下账户入口；用鼠标和键盘依次进入五个子项；切换账号后再次打开；在
  1180×760、1280×768、1280×800、1440×900 和 720px 窄窗口检查布局。
- **交互步骤**：检查“账号与订阅”“模型供应商”“后台运行”“使用指南”“高级诊断”；从
  Host 状态深链后台页；从 Agent 返回后重新打开账户中心。
- **预期输出**：只有当前子项具有 active 和 `aria-current=page`；账号页展示订阅/credits 与
  明确退出按钮；Provider 只在首次进入其子页时读取 Snapshot，且默认仍是已配置列表；后台页
  展示 Host 状态和登录启动；指南可重开状态化 onboarding；高级诊断默认折叠技术信息；菜单
  标题至少 13px、说明至少 12px、点击目标不小于 44px，全部视口无水平溢出；账号切换关闭
  设置中心并重置到“账号与订阅”，旧账号 Provider 状态不得投影到新账号。
- **失败与恢复**：Provider 或诊断不可用不影响其余设置和 Agent 对话；未知设置子路由回退到
  “账号与订阅”，不会显示空白页或同时点亮多个子项。
- **验证层**：Node + Electron + 安装包 + 人工
- **本轮结果**：待执行
- **证据说明**：账户中心专项 Electron 已覆盖五项、键盘、懒加载、账号竞态和五档视口；新包
  动态验收与 owner 人工确认仍待执行。
