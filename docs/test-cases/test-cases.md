...
| 左侧导航与引导 | Agent、模型供应商、设置、Host 状态、三步引导 | 默认落点、切页、Host 三态、加载/失败、零 Provider、零 Agent、激活/失效/回访、恢复引导 | Node + Electron + 安装包；TC-NAV-001～003、TC-GUIDE-001～002 |
...
- **验证层**：Node + Electron + 安装包 + 人工
- **本轮结果**：通过
- **证据说明**：源码 Node、Electron、视觉与产品旅程通过；新包内真实 Host `239/239`，
  `app.asar` 与提交源码逐字节一致，DMG/ZIP/receipt 通过。当前真实安装因禁止自动覆盖而未改动，
  owner 安装后的账号态 UAT 不冒充为已执行。
