...
- 删除“开始使用”和动态下一步标题，把三步有序列表提升为引导卡唯一主内容；
- 整卡保持纯提示，不新增伪按钮或隐式点击；使用 DOM、计算样式和截图回归验证顺序、字号、
  无交互控件与常用窗口适配；
- 复跑相关 Electron/产品旅程，完成 KimiCLI 独立复核、唯一内部包、真实安装验收与进展关闭；
- 严格不扩展 Provider/激活/会话业务逻辑，不新增 onboarding 状态机、P7/P8、签名或发布。

当前进展：实现与交叉测试已完成。首页引导已变为固定语义化三步列表，卡内无按钮、链接或
动态下一步标题；Agent 首页不再为该提示预取 Provider 快照。自主回归通过语法检查、147 项
...
1. Composer 从“附件/输入行/按钮组/说明 Footer”多层表单收敛为单一 30px 大圆角面板；上层
   文字画布取消独立边框与底色，下层工具栏把“＋”与运行态发送意图放左侧，停止、听写和圆形
   箭头发送放右侧，隐私/意图说明降为中间辅助信息；
2. 发送、停止、听写和添加入口改用同一线性 SVG 语言，移除每个工具各自套框的视觉噪声；
   图标仍有稳定中文 `aria-label`、title、键盘焦点和隐藏文本名称，不用纯图形替代可访问语义；
3. 运行中三种意图仍是“调整当前任务 / 排队等待 / 立即执行”，只是从发送按钮的附属窄下拉
   移到左侧轻量胶囊；附件、链接、命令、Skill、工作文件、历史、粘贴卡、本机听写、草稿、
   Queue 与独立 Cancel 的 Main/Host 合同均未改变；
4. 用户指定的隐私文案继续精确保持
   `附件仅在本机暂存，发送时交给当前模型；不会上传到 AgentMesh360`，页面不新增 Core、Host、
   Session、路径或 Key 投影；
5. 新增 `TC-CONV-027`，正式覆盖单体容器、上层输入/下层工具栏、圆形箭头、停止方块、中文
   可访问名称、十附件、运行态、Disabled、四档小屏以及原能力不回退。

当前自动化证据：

- Desktop syntax 与 `git diff --check` 通过；全量 Node `232 total / 227 passed / 0 failed /
  5 skipped`，5 条仍是需要打包 Host 的显式门禁；
- Conversation Electron 真实点击通过，覆盖发送、附件、链接、听写、三种运行态意图、Queue、
  停止防重、Agent 切换和失败草稿恢复；新版发送/停止图标的隐藏文本与 ARIA 同步通过；
- 视觉 smoke 在 1180×760 通过单体布局、30px 圆角、无内层输入边框、42×42 圆形箭头与
  15px 输入字号断言；
- 四档紧凑布局 1180×760、1280×768、1280×800、1440×900 全部退出 0；挂满十个附件时
  Composer 高 209px、Footer 高 42px，bottom 分别为 742、750、782、882px，Transcript 保持
  自身滚动且输入区完整可见；
- 产品旅程验证器 `3/3`，当前 `90` 条、`14` 个领域通过结构与执行检查；自动化使用 fake Host
