...
- [x] 生成并完整性验收唯一内部包，删除旧包；
- [ ] owner 从本地包手动覆盖安装后执行真实账号 UAT。
...
内部 release Host、唯一 DMG/ZIP、摘要和留存门已经关闭。下一步由 owner 从本地包手动
安装，沿“Provider → 激活 Agent → 带附件对话”做真实 UAT；在这之前不把归档完整性写成
安装通过。下一产品切片回到既定计划，不扩展为音视频、云存储或消息队列。
