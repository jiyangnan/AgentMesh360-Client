...
状态：V2.2–V2.6 与 macOS 本机听写迁移、自动化和唯一内部包已完成；
...
- [x] 将听写从模型 Provider 路由迁移到强制 on-device 的 macOS 本机 Helper；
- [x] 保留首击披露、只回填草稿、60 秒上限，并补齐切 Agent/账号/休眠/退出清理；
- [x] 增加 Helper 协议、权限、能力失败关闭、篡改与四档小屏自动化；
- [x] 完成 Node、Rust、Electron、产品旅程、13 寸与重启/重连回归；
- [x] 更新项目进展并生成唯一内部包；全部复验通过后再删除旧包。

内部交付结果：产品 commit `1d708492ef50e75647d56b2fe5ee8a570a530ad2`，回执
`desktop_internal_p6_1d708492ef50_arm64`；DMG/ZIP、包内 Helper、权限说明、arm64 架构、
系统动态库、构建/Downloads 一致性和单包留存均已复验。下一步只执行 owner 真实安装权限与
断网听写 UAT，不继续扩展输入功能。
...
- 不把听写绑定 DeepSeek、GLM、MiniMax 或任何 Agent 推理 Provider；
- 不在本机模型不可用时自动降级到 Apple 云端或额外付费 STT；
