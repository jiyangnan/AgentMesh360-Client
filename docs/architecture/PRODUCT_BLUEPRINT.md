...
| 动态 Agent Package | **H0/H1 至 H2d4 与 P1-P4 已完成；P5 v2 已通过 owner OAuth/BYOK，Release Chain 预检、双代 Builder、隔离基础设施与 Origin 部署执行器已冻结** | H2d4 已在 Artifact/Envelope 前 bounded fetch Release Manifest，核对 Registry SHA、canonical strict 文档、Client/Host projection 与 H1 metadata。P2/P3/P4 分别完成测试 key ceremony、四 Agent 双构建 provenance 与隔离分发演练并清场；P5 已固定四 Agent baseline、Job 同权限/权限扩张版本、两 Publisher、单 SGP1 Droplet、双 Spaces bucket、DNS-only Origin 部署和异常恢复，但尚未实际生成本轮临时签名 key、创建 staging、发布或执行 21 场景。生产 Root、Publisher Bundle、endpoint、上传和发布仍关闭 |
