...
| 桌面产品外壳 | **身份外壳、Agent 首页、Provider/Package/客户端设置，以及 Host Catalog 全部 Agent 的固定 Main Session 文本对话、恢复通路、标准 ACP 单次权限审批、安全只读工具活动、通用 Workspace Artifact/Project State 与 Harness 后台活动安全投影已实现** | 三个首方 Agent 已通过真实 detach/Leader 替换恢复；动态 Agent 只有本地通用化 fixture，生产 Registry 仍关闭。Artifact/Project State 使用通用 Manifest 与 Renderer 白名单；后台活动使用当前 Main Session 通知与 Host-owned 安全快照对账，均不硬编码 Agent 类型。下一步只审计 ACP Plan/Todo，不直接铺开 Scheduler、Subagent 或 Agent 专属 UI |
...
`x.ai/task/list`、命令、cwd、输出、日志、Scheduler、Plan/Todo 和 Subagent 均未
暴露或混入。下一产品项只在独立契约中评估 ACP Plan/Todo 是否值得成为 Session
计划视图，不直接开启 Agent 专属 UI 或新的 Package 生产切片。循环 50 与循环 52
均已完成自主验证和 Kimi 独立复核，最终四级问题全零。生产 Package 与桌面发布继续
受独立 R0-R6 安全门约束，
完整结论见
[`PRODUCT_PLAN_AND_PRODUCTION_RELEASE_GATE.md`](PRODUCT_PLAN_AND_PRODUCTION_RELEASE_GATE.md)。
