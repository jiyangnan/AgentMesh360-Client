...
| BYOK Provider 层 | **切片 A/B/C/D0/D1/E1/E2/E3 与 F0a 已实现** | 已实现共享 state.db v8（v7/v8 只增加并加固 Package Registry）、Provider Profile/Vault、声明式 Catalog、Capability、Model Policy、三层 Model Assignment、非秘密 RouteCompiler、账户隔离产品 Agent、不可变 Session Binding、Turn Route 可信存储接口、管理 ACP、凭据诊断安全门槛、Host Credential Lease、三协议投影、actor 接收后写 Turn Route、同一 Turn 多调用复用、产品主 Prompt与全部已确认 Session 辅助消费者接入；产品 subagent 使用不可伪造的 Host-only `subagent` Authority，不继承 Grok credential/AuthManager，父→子→父本机 mock Provider E2E、专用模型/main fallback 与失败门槛均已覆盖；离线 Trace classifier 已确认不属于产品 Session 数据面；Renderer 已获得订阅门禁、输入校验、输出脱敏的 Host Provider 管理窄桥，并提供 Profile/Catalog/global-agent Assignment 设置页；E3 已加入 Host-owned 本地/元数据/最小推理 Probe、付费双重确认与非秘密历史；F0a 已加入默认零费用的 OpenAI Chat Provider 契约 Harness 与双重 opt-in Gemini 真实入口，并确认 thought signature 跨轮保真是正式 Catalog 阻断项；外部真实 Provider E2E 仍待用户凭据 |
| 动态 Agent Package | **H0/H1、H2a1、H2a2 已实现并通过交叉测试；H2b 待实现** | v1 Manifest、内置只读 Catalog、Registry/Profile/Session/Workspace/Model Policy 投影、旧状态兼容、Ed25519 strict 签名信封、Artifact 单文件句柄验签/解包、锚定文件清单摘要、安全 staging、Archive entry/文件/体积/路径边界，以及 Active/Previous 原子安装、权限增量批准、稳定身份、回滚前整树复验和 v8 本地 Registry 已实现；H2a1 已让 Host 启动时复验 Active，确定性合并内置升级与新 Agent；H2a2 让 Runtime/Model Routing/产品 Turn 复用原子共享 Catalog 快照，增加失败保留最后可信快照的 Host 私有 refresh，以及订阅门禁、原始错误/路径/秘密脱敏的 built-in/Active/Previous/invalid/orphan 只读状态；生产信任根、安装 mutation、远端目录、迁移、orphan 清理与宿主 Skill 安装仍关闭 |
...
Active Package；由于生产 Trust Store 和安装入口仍关闭，真实用户目录当前仍只会使用
三个内置 Package。显式 refresh 与不暴露 mutation 的只读管理状态已经通过自主测试和
两轮本机 Kimi 交叉测试；下一阶段接生产 Trust Store、远端 Registry、权限 UI 和宿主
Skill 安装投影。
