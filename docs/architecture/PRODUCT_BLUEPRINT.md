...
| 动态 Agent Package | **H0/H1、H2a1、H2a2、H2b0、H2b1a、H2b1b1、H2b1b2、H2b1c、H2b2a、H2b2b 已通过自主测试和 Kimi 交叉测试** | H2b2b 已加入 Host 内存一次性审批、账户/TTL/容量边界、完整 install plan 绑定、批准时 staging 与当前 Active 重验、既有 immutable versions + SQLite CAS 所有权交接，并删除任意路径/裸布尔安装入口。H2b2c 已开始审计安装后的 Shared Runtime Catalog 原子可见性；ACP/UI 与生产 endpoint/root/bundle 继续关闭 |
