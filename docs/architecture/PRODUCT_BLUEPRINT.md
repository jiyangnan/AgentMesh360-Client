...
| 动态 Agent Package | **H0/H1 至 H2c1 已通过自主测试和 Kimi 交叉测试** | Host ACP 已提供订阅门禁、严格输入、跨账户批准隔离、防重放、脱敏错误与桌面主进程按 ID 窄 Client；Renderer UI 与生产 endpoint/root/bundle 继续关闭。下一步进入 H2c2 Package Center 与显式权限批准 |
...
自主测试和 Kimi 独立交叉测试；下一步 H2c2 只实现桌面 Package Center 和显式权限
批准，生产 Trust Store 与宿主 Skill 安装投影继续作为后续独立发布门。
