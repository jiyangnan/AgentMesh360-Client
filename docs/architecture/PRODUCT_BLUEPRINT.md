...
| 动态 Agent Package | **H0/H1、H2a1、H2a2、H2b0、H2b1a 已通过自主测试和 Kimi 交叉测试；H2b1b+ 待实现** | H2b1a 已增加 root-signed Registry Snapshot 纯验证器，把 Package/Agent/版本/Publisher、Artifact/Envelope canonical HTTPS URL 与摘要、Trust Bundle sequence/root 绑定到单调 revision，并拒绝无 active key 的 publisher；生产 root/bundle、可信 server time、持久最高 sequence/revision、远端抓取/cache、安装 mutation、迁移、orphan 清理与宿主 Skill 安装仍关闭 |
