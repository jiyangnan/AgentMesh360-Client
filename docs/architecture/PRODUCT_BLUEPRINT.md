...
| 动态 Agent Package | **H0/H1 至 H2b2d 已通过自主测试和 Kimi 交叉测试** | install/rollback/reconcile 已共用同一状态根、跨 Registry clone 顺序门、门内 Catalog + health outcome 和脱敏 Mutation Receipt；磁盘 mutation 已提交但 refresh 失败时保留 last-known-good，修复后可显式 reconcile。H2c1 将建立订阅门禁的 Host 管理 ACP；Renderer UI 与生产 endpoint/root/bundle 继续关闭 |
...
测试。下一步先建立 Host 管理 ACP，再分离实现桌面权限 UI、生产 Trust Store 和宿主
Skill 安装投影。
