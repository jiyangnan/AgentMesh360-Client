...
下一门槛是把已验证 staging 通过 Active/Previous 原子安装事务接入可回滚本地
Registry，再替换“只能随客户端发版更新目录”的限制。
