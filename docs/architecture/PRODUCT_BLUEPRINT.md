...
| 动态 Agent Package | **H0/H1 至 H2d4 与 P1-P4 已完成；P5 v2 的隔离 Origin、双代 Release Chain 与 Registry-last 发布已真实通过** | H2d4 已在 Artifact/Envelope 前 bounded fetch Release Manifest，核对 Registry SHA、canonical strict 文档、Client/Host projection 与 H1 metadata。P2/P3/P4 分别完成测试 key ceremony、四 Agent 双构建 provenance 与隔离分发演练并清场；P5 已固定四 Agent baseline、两个 Job 更新、两 Root/Publisher、四段 Trust、五版 Registry、单 SGP1 Droplet、双 Spaces bucket、DNS-only Origin、Registry-last 发布、14 项 live Host 加 7 项证据/契约场景和 Registry-first 全清场顺序。隔离 Spaces、limited principal、唯一 Droplet、DNS-only/TTL 60、非特权 HTTPS Origin、双代六个 Release 可复现构建和 61/61 对象发布均已真实通过；Registry 最后上线，公网 Trust/Registry 与真实 Release 的状态、MIME、无重定向和摘要均通过。21 场景已真实暴露并修复三项消费侧问题：严格 P5-only、保留 TLS hostname 的 Fake-IP Origin 覆写；签名 expiry 毫秒缓存精度；Package runtime receipt camelCase 与动态本地 Catalog revision 的 JS 安全整数。两 Root/两 Publisher 暂留待场景与清场，生产 Root、Publisher Bundle、endpoint 和发布仍关闭 |
