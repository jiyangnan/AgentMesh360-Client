...
| 动态 Agent Package | **H0/H1、H2a1、H2a2、H2b0、H2b1a、H2b1b1 已通过自主测试和 Kimi 交叉测试；H2b1b2+ 待实现** | H2b1b1 已把 Trust Bundle/Registry 验签时间收口到当前 Granted `ClientAccess`：Core `server_time` 只由单调 `Instant` 推进，最多 10 分钟且不超过订阅剩余时间；墙钟只检测超过 2 分钟的漂移/回退并失败关闭。生产 root/bundle、持久最高 sequence/revision、远端抓取/cache、安装 mutation、迁移、orphan 清理与宿主 Skill 安装仍关闭 |
