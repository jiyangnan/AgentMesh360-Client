...
| 桌面产品外壳 | **身份外壳、Agent 首页、Provider/Package/客户端设置，以及 Host Catalog 全部 Agent 的固定 Main Session 文本对话、恢复通路、标准 ACP 单次权限审批、安全只读工具活动、通用 Workspace Artifact/Project State、Harness 后台活动与 Session Plan 安全投影已实现；P6 R4 no-authority preflight 和 unsigned internal 构建合同已完成** | 三个首方 Agent 已通过真实 detach/Leader 替换恢复；P6 内部子阶段只允许 clean pushed commit 生成当前架构 DMG/ZIP，固定无 Developer ID、无公证、无自动更新/上传、receipt + SHA-256 和单应用“仍要打开”说明。下一步先执行真实内部 build 与安装/生命周期矩阵；生产 R4、18 项签名/更新/rollback 场景、Scheduler、Subagent、Agent 专属 UI 和生产 authority 不自动启动 |
