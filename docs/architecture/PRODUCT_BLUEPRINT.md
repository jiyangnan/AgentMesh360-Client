...
| 动态 Agent Package | **H0/H1 已实现，H2 待实现** | v1 Manifest、内置只读 Catalog、Registry/Profile/Session/Workspace/Model Policy 投影、旧状态兼容、Ed25519 strict 签名信封、Artifact 单文件句柄验签/解包、锚定文件清单摘要、安全 staging、Archive entry/文件/体积/路径边界，以及 Active/Previous 原子安装、权限增量批准、稳定身份、回滚前整树复验和 v8 本地 Registry 已实现；生产信任根与管理入口仍关闭，动态运行时合并、远端目录、迁移、orphan 清理与宿主 Skill 安装待实现 |
...
分别硬编码 Registry 元数据与 Agent Profile。当前 Catalog 仍是编译期只读内置目录；
下一门槛是把可回滚本地 Registry 的 Active Package 安全合并进运行时 Catalog，并在
不暴露 mutation 的前提下先提供只读状态；之后再替换“只能随客户端发版更新目录”的
限制。
