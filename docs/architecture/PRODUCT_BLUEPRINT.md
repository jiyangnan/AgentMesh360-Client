...
| 动态 Agent Package | **H0/H1 至 H2d4 与 P1-P4 已完成；P5 v2 已通过 owner OAuth/BYOK 和 Release Chain 无网络预检** | H2d4 已在 Artifact/Envelope 前 bounded fetch Release Manifest，核对 Registry SHA、canonical strict 文档、Client/Host projection 与 H1 metadata。P2/P3/P4 分别完成测试 key ceremony、四 Agent 双构建 provenance 与隔离分发演练并清场；P5 当前只关闭真实订阅/BYOK 和冻结执行前门，尚未生成本轮临时签名 key、创建 staging 或执行 21 场景。生产 Root、Publisher Bundle、endpoint、上传和发布仍关闭 |
