...
| 动态 Agent Package | **H0/H1、H2a1、H2a2、H2b0、H2b1a、H2b1b1、H2b1b2、H2b1c、H2b2a 已通过自主测试和 Kimi 交叉测试** | H2b2a 只按已重新验签的 Registry Record 下载 Envelope/Artifact：固定生产 HTTPS origin、禁重定向、64 KiB/32 MiB 双层上限、流式 digest、私有逐操作 staging、符号链接防护、既有 Artifact 验证器复用和脱敏审计；仍不移动到 versions、不改变 Active/Previous/Catalog、不开放安装 mutation/权限 UI。生产 endpoint/root/bundle 继续为空；H2b2b 再接权限审批和既有本地安装事务 |
