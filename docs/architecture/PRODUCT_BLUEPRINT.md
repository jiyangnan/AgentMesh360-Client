...
| 动态 Agent Package | **H0/H1 至 H2d2 已通过自主测试和 Kimi 独立交叉测试** | H2d2 已用不可伪造 H2d1 receipt 将 H1 Artifact/Envelope、签名 plan/projection 与全部 Host bundles 绑定为 strict canonical Agent Release Manifest；H1/H2d2 共用身份与路径边界。动态新 Agent 与零 Adapter 均无需专属代码。私钥、网络和生产发布配置保持关闭；下一步 H2d3 建立受签名 Release 发布索引与双渠道只读投影 |
