...
| 动态 Agent Package | **H0/H1 与 H2a1 已实现，H2a2/H2b 待实现** | v1 Manifest、内置只读 Catalog、Registry/Profile/Session/Workspace/Model Policy 投影、旧状态兼容、Ed25519 strict 签名信封、Artifact 单文件句柄验签/解包、锚定文件清单摘要、安全 staging、Archive entry/文件/体积/路径边界，以及 Active/Previous 原子安装、权限增量批准、稳定身份、回滚前整树复验和 v8 本地 Registry 已实现；H2a1 已让 Host 启动时复验 Active，确定性合并内置升级与新 Agent，并让 AgentDefinition、Model Policy、Session/Workspace 消费合并 Catalog；生产信任根与管理入口仍关闭，显式 refresh、只读状态、远端目录、迁移、orphan 清理与宿主 Skill 安装待实现 |
...
三个内置 Package。下一门槛是显式 refresh 与不暴露 mutation 的只读管理状态，之后再
接远端 Registry 和宿主 Skill 安装投影。
