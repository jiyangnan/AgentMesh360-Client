...
| 动态 Agent Package | **H0/H1 至 H2d4 与 P1-P4 已完成；P5 v2 已通过 owner OAuth/BYOK，Release Chain、隔离 staging、双代发布、21 场景与 Registry-first 清场执行器均已冻结；隔离 Spaces、最小权限 principal、唯一 Droplet、DNS-only 地址与 HTTPS Origin 已真实通过** | H2d4 已在 Artifact/Envelope 前 bounded fetch Release Manifest，核对 Registry SHA、canonical strict 文档、Client/Host projection 与 H1 metadata。P2/P3/P4 分别完成测试 key ceremony、四 Agent 双构建 provenance 与隔离分发演练并清场；P5 已固定四 Agent baseline、两个 Job 更新、两 Root/Publisher、四段 Trust、五版 Registry、单 SGP1 Droplet、双 Spaces bucket、DNS-only Origin、Registry-last 发布、14 项 live Host 加 7 项证据/契约场景和 Registry-first 全清场顺序。当前已真实创建两个隔离 Bucket、Publisher/Origin limited Key、唯一最小 SGP1 Droplet 与一条 DNS-only/TTL 60 A 记录，并通过非特权 Origin、Caddy TLS、公网 health、fault probe 和只读 Spaces 路由复验；测试签名 key、上传和 21 场景仍未执行。生产 Root、Publisher Bundle、endpoint 和发布仍关闭 |
