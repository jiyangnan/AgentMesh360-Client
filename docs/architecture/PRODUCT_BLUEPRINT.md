...
| 桌面产品外壳 | **身份外壳、Agent 首页、Provider/Package/客户端设置，以及 Host Catalog 全部 Agent 的固定 Main Session 文本对话、恢复通路、标准 ACP 单次权限审批、安全只读工具活动、通用 Workspace Artifact/Project State、Harness 后台活动与 Session Plan 安全投影已实现；P6 R4 no-authority preflight 已完成** | 三个首方 Agent 已通过真实 detach/Leader 替换恢复；P6 已把当前 unsigned DMG/ZIP、缺失 Developer ID/公证/entitlement/electron-updater/更新渠道的事实固化为 blocked 契约，并固定 18 项签名、安装、生命周期、更新、rollback 与卸载场景。真实 Desktop Candidate 仍需 Apple 凭据、签名/公证和更新渠道的独立授权；Scheduler、Subagent、Agent 专属 UI 和生产 authority 不自动启动 |
