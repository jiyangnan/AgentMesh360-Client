...
| 桌面产品外壳 | **身份外壳、Agent 首页、Provider/Package/客户端设置，以及 Host Catalog 全部 Agent 的固定 Main Session 文本对话、恢复通路和标准 ACP 单次权限审批已实现** | 三个首方 Agent 已通过真实 detach/Leader 替换恢复；动态 Agent 只有本地通用化 fixture，生产 Registry 仍关闭。下一步只做安全、只读的 Harness 工具活动状态投影，产物和垂直工作区继续后置 |
...
2026-07-27 的循环 43-45 已依次关闭固定 Main Session 文本对话、多 Agent 恢复和
标准 ACP `session/request_permission` 单次权限边界。Main 独占 Session、Request、
Tool 与 Option authority，Renderer 只得到有界安全投影；永久或未知审批选项失败
关闭。下一产品项只审计并实现安全、只读的 Harness 工具活动状态投影，不直接开启
新的 Package 生产切片。生产 Package 与桌面发布继续受独立 R0-R6 安全门约束，完整
结论见
[`PRODUCT_PLAN_AND_PRODUCTION_RELEASE_GATE.md`](PRODUCT_PLAN_AND_PRODUCTION_RELEASE_GATE.md)。
