...
| 动态 Agent Package | **H0/H1 至 H2b2c 已通过自主测试和 Kimi 交叉测试** | 安装 CAS 与 Shared Runtime Catalog refresh 已进入同一跨 Registry clone 顺序门；Receipt 区分 visible/superseded/refresh_pending，刷新失败保留 last-known-good，新 Agent 可在既有账户后续列表/激活中投影。H2b2d 将补齐 rollback/恢复的一致语义；ACP/UI 与生产 endpoint/root/bundle 继续关闭 |
...
通过自主测试和本机 Kimi 交叉测试；随后先补齐 rollback/恢复的一致性，再接生产
Trust Store、权限 UI 和宿主 Skill 安装投影。
