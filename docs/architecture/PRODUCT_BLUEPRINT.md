...
| 动态 Agent Package | **H0/H1、H2a1、H2a2、H2b0 已通过自主测试和 Kimi 交叉测试；H2b1+ 待实现** | 既有 Manifest/Artifact/安装/运行时能力不变；H2b0 新增客户端 root 签名的 Publisher Trust Bundle v1，覆盖双 active key 轮换、retired/revoked、bundle/key 时间窗、单调 sequence 验证参数、canonical Base64/确定性 payload/Ed25519 strict verification 与非秘密审计投影。生产 root/bundle、持久最高 sequence、远端 Registry/cache、安装 mutation、迁移、orphan 清理与宿主 Skill 安装仍关闭 |
