...
| 桌面产品外壳 | **身份外壳、Agent 首页、Provider/Package/客户端设置，以及 Host Catalog 全部 Agent 的固定 Main Session 文本对话、恢复通路、标准 ACP 单次权限审批、安全只读工具活动、通用 Workspace Artifact 与 Project State 最小投影已实现** | 三个首方 Agent 已通过真实 detach/Leader 替换恢复；动态 Agent 只有本地通用化 fixture，生产 Registry 仍关闭。Artifact/Project State 使用同一通用 Manifest 与 Renderer 白名单，不硬编码 Agent 类型；下一步只审计任务与后台活动 authority，不直接铺开 Agent 专属 UI |
...
2026-07-27 的循环 43-50 已依次完成固定 Main Session 文本对话、多 Agent 恢复、
标准 ACP `session/request_permission` 单次权限边界、ToolCall 安全只读活动投影，
Workspace Artifact，以及 Project State authority 审计和通用最小只读实现。
...
步骤，路径、文件内容和 mutation 仍关闭。下一产品项只审计任务与后台活动的稳定
Harness authority、恢复与脱敏边界，不直接开启 Agent 专属 UI 或新的 Package 生产
切片。循环 50 已完成自主验证和 Kimi 独立复核，最终四级问题全零。生产 Package 与
桌面发布继续受独立 R0-R6 安全门约束，
完整结论见
[`PRODUCT_PLAN_AND_PRODUCTION_RELEASE_GATE.md`](PRODUCT_PLAN_AND_PRODUCTION_RELEASE_GATE.md)。
