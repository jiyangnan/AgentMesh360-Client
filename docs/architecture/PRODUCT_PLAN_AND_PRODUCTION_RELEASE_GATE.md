...
2026-08-05 owner 已在新客户端包的原持久 Job Agent 链路中复跑真实猎聘并确认顺利投递，
因此本切片与 `TC-AGENT-009` 正式完成。该证据只覆盖本次安装版端到端路径，不泛化为所有
账号、城市、岗位或招聘站点；Client 仍不拥有猎聘抓取实现。按既定顺序，下一独立产品切片
...
1. 复用现有 `agent:get-model-overview`，只增加 `configuredProviderCount` 非秘密字段；它只
   统计已配置凭据且至少有一个启用模型的 Profile，不把完整 Provider Snapshot 或 Key 交给
   首页；
...
4. Agent 激活返回后必须由 Main 确认身份仍为 ready、无激活错误、目标
   ...
   不能误显示成功或打开对话；启动中的 Agent 只显示后台进度，不阻塞切换已有 Agent；
5. 自动化覆盖加载、失败重试、零 Provider、零 Agent、未激活、失效、正常/失效混合、回访、
   激活失败、迟到旧账号响应、1180×760 与窄窗口，并更新安装包动态验收；安装脚本按权威
   Overview 精确核对 CTA ID、文案和目标 Agent，不再沿用 2026-08-02 的零按钮合同。
