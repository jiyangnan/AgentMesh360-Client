...
1. 对尚未走通主路径或需要修复配置的用户，显示状态化三步路径：
   - 添加模型供应商；
   - 激活 Agent；
   - 在 Agent 对话中开始工作；
...
三步引导只根据 Host 的非秘密模型 Overview 和 Agent 常驻状态判断“待确认 / 当前步骤 /
已完成 / 待完成”，每个 ready 状态只提供一个真实下一步按钮。按钮复用左侧“模型供应商”、
Agent 卡片、Agent 模型设置和主对话既有路由，不建立第二套业务流程。Overview 读取中不猜
“未配置”，读取失败只允许重试；Catalog 为空不能显示静默无动作 CTA。已有正常常驻 Agent
的回访用户首页不再显示新手卡，“设置 > 使用指南”仍展示同一状态并允许打开或继续明确命名
的 Agent。
